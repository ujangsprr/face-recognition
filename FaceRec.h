...
void addFace()
{
    ...
    if (!face_cascade.load("include/haarcascade_frontalface_alt.xml"))
    {
        cout << "error" << endl;
        return ;
    };
...
void eigenFaceTrainer() {
	...
	model->save("include/eigenface.yml");
...
void FaceRecognition() {
	...
	model->read("include/eigenface.yml");

	Mat testSample = imread("images/example.jpg", 0);
